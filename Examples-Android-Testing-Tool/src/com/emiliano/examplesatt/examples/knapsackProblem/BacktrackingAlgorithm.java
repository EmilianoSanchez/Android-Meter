...
public class BacktrackingAlgorithm implements Component<KnapsackInstance,KnapsackSolution>{
	...
	private static void backtracking(KnapsackInstance input, boolean[] bestSolution,int[] bestSolutionValue, boolean[] currentSolution, int index, int acumWeight) {
		if(index<input.itemWeights.length){
			if(acumWeight<=input.knapsackWeight){
				...
				backtracking(input,bestSolution,bestSolutionValue,currentSolution,index+1,acumWeight+input.itemWeights[index]);
